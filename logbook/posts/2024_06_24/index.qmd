...
Succesful reproduction of Table 6. Total time used: TBCh TBCm (TBC%)
...
Add standard deviation. Some percent change seem large as it's comparing very small number - but actual change between the model and table 6 all are small enough that I feel happy to say this has been successfully reproduced.

Will run 100 replications later (if run now, will prevent from working on other things). However, as would otherwise run now, if that comes out as matching up too, then would say timestamp at which this was complete is 16.02 + a few minutes, as that's when I would've otherwise finished up with it if possible.

**Therefore, timestamp at which feel has been reproduced:** 16.02 + 5 min (for checking table later)

**Timing at that point:** 32.8%, 787m

```
Time spent today: 256m, or 4h 16m
Total used to date: 787m, or 13h 7m
Time remaining: 1613m, or 26h 53m
Used 32.8% of 40 hours max
```
