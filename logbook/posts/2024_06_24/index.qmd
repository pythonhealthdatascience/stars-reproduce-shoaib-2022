...
To ensure it is not an error I've introduced, I copied the original `PHC.py` and modified it directly (the only change from config 1 that I have identified from the paper is that the OPD IAT is 3 instead of 4), and to produce both results spreadsheets. Saved all within `logbook/` and renamed, so can refer to from this post.

This came out looking the same as my other run (e.g. doctor utilisation around 0.31 instead of 1.14).

```{python}
import pandas as pd
import xlrd

book = xlrd.open_workbook('direct_c4_rep.xls')
direct_c4 = pd.read_excel(book, header=None, index_col=0)
direct_c4
```

```{python}
direct_c4_full = pd.read_excel('direct_c4_full.xlsx', header=None).rename(columns={
    0: 'full_outcome',
    1: 'full_result'})
direct_c4_full
```
